...
def preprocess(df):
    ...
    bin_maps = {'tolazamide':{'No':0,'Steady':1,'Up':2},'acetohexamide':{'No':0,'Steady':1},'glimepiride-pioglitazone':{'No':0,'Steady':1},'metformin-pioglitazone':{'No':0,'Steady':1},'metformin-rosiglitazone':{'No':0,'Steady':1},'glipizide-metformin':{'No':0,'Steady':1},'tolbutamide':{'No':0,'Steady':1},'change':{'Ch':1,'No':0},'diabetesMed':{'Yes':1,'No':0}}
...
def main():
    ...
    keep = variances.index[:200]
    X_reduced = X[keep]
    X_test_reduced = X_test[keep]
    X_tr, X_val, y_tr, y_val = train_test_split(X_reduced, y, test_size=0.2, random_state=42, stratify=y)
    ...
    print('Validation f1_macro:', round(f1,4))
    ...
    print('Wrote outputs/kaggle_submission.csv (fast model).')
